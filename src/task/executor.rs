...
use futures_util::task::{Waker, Context, Poll};
use alloc::task::Wake;
...
impl Executor {
    ...
    pub fn spawn(&mut self, task: Task) {
        ...
        if self.tasks.insert(task_id, task).is_some() {
            panic!("A task with the same ID is already in the tasks queue.")
        }
        self.task_queue.push(task_id).expect("The queue is full.")
    }
    ...
    fn run_ready_tasks(&mut self) {
        // destructure `self` to avoid borrow check errors
        let Self {
            tasks,
            task_queue,
            waker_cache
